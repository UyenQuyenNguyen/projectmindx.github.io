...
const but = document.querySelectorAll("#header-menu > div > a > button")
let onScroll = () => {
    ...
    function callBackFunc() {
        ...
        if (y > 10) {
            ...
            document.querySelector("#header-container > h1").style.color = "black";
        } else {
            ...
            document.querySelector("#header-container > h1").style.color = "white";
...
// let listOld = document.getElementById('dropdown-ex');
// function exData() {
//     let child = [];
//     for (let i = 0; i < 5; i++) {
//         child.push(JSON.stringify(dataSearchStorage.pop().title));
//     }
...
function renderProduct() {
    ...
    const content = product.map((item, index) => {
        if (index >= start && index < end) {
            ...
            html += '<a href = "http://127.0.0.1:5500/CodePlay/DetailProduct.html">';
            html += '<img src = ' + item.image + '>';
            html += '</a>';
            html += '<div class="description"><h6>' + item.title + '</h6>';
            html += '<p><del>' + (parseInt(item.price) + (parseInt(item.price) * 0.2)) + '.00$</del> ' + item.price + '</p>';
            html += '</div></div></div>';
        }
    });
...
function renderByClasstify(classtifyClick) {
    let html = "";
    const content = product.map((item, classify) => {
...
let dataSearchStorageShop = localStorage.getItem('search-keyword') ? JSON.parse(localStorage.getItem('search-keyword')) : [];
function renderSearchProductShop() {
    if (dataSearchShop.value != "") {
        dataSearchStorageShop.push({
            title: dataSearchShop.value,
            type: "keyword"
        });

        localStorage.setItem('search-keyword', JSON.stringify(dataSearchStorageShop));
        let html = "";
        const content = product.map((item) => {
            if (item.classify.includes(dataSearchShop.value) || item.title.includes(dataSearchShop.value)) {
                html += '<div class="col">'
                html += '<div class="detail-item">';
                html += '<a href = "http://127.0.0.1:5500/CodePlay/DetailProduct.html">';
                html += '<img src = ' + item.image + '>';
